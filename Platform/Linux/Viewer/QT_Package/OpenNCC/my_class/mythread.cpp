...
void MyThread::load_2net_model(int Video_type,int modeId,int h26x_type)
{
    ...
    if (ret<0)
    {
        ...
        emit send_log_info_to_UI(tr("list.num:%1").arg(list.num));
        ...
        if (ret<0)
        {
            ...
            while(two_net_model)
            {
                ...
                if(fps_log%10==0)
                {
                    static QElapsedTimer *lostTime = NULL;
                    if(NULL==lostTime)
                    {
                        lostTime = new QElapsedTimer;
                    }
                    else
                    {
                        T = lostTime->elapsed();
                ...
                if(YUV420p==Video_type)
                {
                    pFrame = (char*)recvImageData + sizeof(frameSpecOut);
                }
                metaData = (char*)recvMetaData + sizeof(frameSpecOut);
                sprintf(src,"demo_video_%dx%d@%dfps",cameraCfg.camWidth,cameraCfg.camWidth,cameraCfg.camFps);
            ...
            while(1)
            {
                ...
                sprintf(src,"demo_video_%dx%d@%dfps(scale:%d%%",cameraCfg.camWidth,cameraCfg.camWidth,cameraCfg.camFps,(int)(100*scale));
                vehicle_license_plate_detection_barrier(yuv420p,cameraCfg.camWidth,cameraCfg.camHeight,scale,src,true,&cnn1PrmSet,&cnn2PrmSet,metaData,min_score,hdr.res[8],1,m_id,showstate,30);
            }
#endif
            streamrun=0;
            //run finsh
...
>>>>>>> fc9161bacadc6b6faf9681f489ba8d0419aa9042
