...
void  segmentation_func(void *data, int w, int h, float scale, char *name, int nn_fov_show, CameraInfo *nnparm, char *nnret,float min_score);
...
int main(void)
{
   ...
   //5.1 Configure the effective area of the algorithm
    ...
    cam_info.inputDimWidth  = 896;
    cam_info.inputDimHeight = 512;
    ...
    const char *blob = "./blob/2020.3/road-segmentation-adas-0001/road-segmentation-adas-0001.blob";
    ...
    while (1)
    {
       ...
       // sprintf(src, "FD_video_%dx%d@%dfps(scale:%d%%)", cameraCfg.camWidth,
        ...
        segmentation_func(yuv420p, cameraCfg.camWidth, cameraCfg.camHeight,scale, src, 1, &cam_info,memedata , min_score);
    }
