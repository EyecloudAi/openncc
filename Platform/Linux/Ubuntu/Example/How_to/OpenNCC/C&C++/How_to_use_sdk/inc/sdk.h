...
enum usb_error {
	...
	/*add by duke 2021.11.11*/
	EC_ERROR_DATA_EMPTRY =-20,
...
typedef struct{
  ...
  UINT8  extInputs[MAX_EXTINPUT_SIZE];       /* second model input   */
...
typedef struct{
    ...
    int endXAdj;                                    
    int endYAdj;
    UINT8  labelMask[MAX_LABEL_SIZE];          /* mask label, bit equal to 1  will be useful */
...
int GetJpegData(char *pbuf, int slen);
int GetOneAIData(char *pbuf, int slen);  //read one ai mode ai data,frameSpecOut+float array
...
int read_meta_image_data( char *buf_metadata, int *len_metadata, char *buf_jpg, int *len_jpg, int blocked);
...
int GetMetaDataExt(char *pbuf, int slen,int* outsize0,int *outsize1,int* item_num);

//2 net model 
int sdk_net3_init(vscRecvCb cb,void* param, const char *blob_path, Network1Par *par, int par_Len,
 const char *blob2_path, Network2Par *par2, int par2_Len,
 const char *blob3_path, Network2Par *par3, int par3_Len);
