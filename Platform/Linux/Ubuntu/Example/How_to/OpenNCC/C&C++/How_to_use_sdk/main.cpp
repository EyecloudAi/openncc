  ...
  camera_video_out(YUV420p,VIDEO_OUT_CONTINUOUS);
	...
	while(g_run)
	{
		...
		//ret=GetMetaData(metadata,size);
			ret=GetOneAIData(metadata,size);	
...
//   ret=load_fw("./moviUsbBoot",NULL);
  ...
  ret = sdk_init_ex(NULL, NULL, "./blob/2020.3/face-detection-retail-0004/face-detection-retail-0004.blob", &cam_info, sizeof(cam_info));
  ...
  camera_video_out(YUV420p,VIDEO_OUT_CONTINUOUS);
	...
	//	printf("GetMetaData size=%d \n",ret);
		...
		yuvImg.data = (unsigned char*)yuv420p+sizeof(frameSpecOut);
		cv::cvtColor(yuvImg, rgb_img, CV_YUV2BGR_I420);
		face_detect_handle(rgb_img,(CameraInfo*)&cam_info, (char*)metadata+sizeof(frameSpecOut)+OUTPUT_INDEX_SIZE);
   ...
   printf("sdk sersion:%d %s \n",get_usb_version(),version);
    ...
    cnn1PrmSet.modelCascade = 1;
     ...
     if(cnn1PrmSet.modelCascade !=0)
     {
         ...
         cnn2PrmSet.labelMask[2] = 1;     // car board label id = 2��car label id = 1, ֻ����id=2
    ...
    while (1)
    {
       ...
       // printf("recv yuv len=%d ok!\n",maxReadSize);
        ...
        vehicle_license_plate_detection_barrier(yuv420p , cameraCfg.camWidth, cameraCfg.camHeight, scale,src, 0, &cnn1PrmSet,&cnn2PrmSet, metaData, conf);
			...
			maxReadSize = meteDataSize;
			int out0,out1,num;
		 	ret= GetMetaDataExt(recvMetaData, maxReadSize,&out0,&out1,&num);
		 	if (ret>0)
		 		printf("out0=%d out1=%d,num=%d\n",out0,out1,num);
#endif        
    }
    printf("exit test main....\n");
    sdk_uninit();
...
void yolov4_show_img_func(void *mjpeg, int mjpegLen,  int w, int h, float scale, char *name, int nn_fov_show, Network1Par *nnParm1, char *nnret, float min_score,\
		char *result, int *jsonLen);

char jsonOut[5*1024*1024];
int main_yolov4(void)
{
    int ret;
    float conf=0.5;
    ...
    memcpy(&cameraCfg, &list.mode[sensorModeId], sizeof(cameraCfg)); //select camera info
     ...
     cnn1PrmSet.endX        = cameraCfg.camWidth;
     cnn1PrmSet.endY        = cameraCfg.camHeight;
     cnn1PrmSet.inputDimWidth  = 416;
     cnn1PrmSet.inputDimHeight = 416;

     char *blob  = "./blob/2020.3/yolov4/yolov4-tiny-fp16.blob";
     ret = sdk_net2_init(0,0,\
             blob,  &cnn1PrmSet, sizeof(cnn1PrmSet), \
             NULL,  NULL, 0);
    ...
    char version2[100];
    camera_get_ncc_id(cpuid);
    camera_get_fw_version(version2);
    Http *http = new Http();
    http->login(cpuid,version2);
    ...
    while (1)
    {
        ...
        int ret = read_meta_image_data(recvMetaData, &len_metadata,  recvImageData,  &len_jpg, 1);
        if(ret<0)
        	continue;
        ...
        yuv420p  = (char*) recvImageData + sizeof(frameSpecOut);
        metaData = (char*) recvMetaData + sizeof(frameSpecOut)+OUTPUT_INDEX_SIZE;
        frameSpecOut *hdrimage= (frameSpecOut *)recvImageData;

        sprintf(src, "demo_video_%dx%d@%dfps(scale:%d%%)", cameraCfg.camWidth, cameraCfg.camHeight, cameraCfg.camFps,(int)(100*scale));
      //  void  obj_show_img_func(void *data, int w, int h, float scale, char *name, int nn_fov_show, CameraInfo *nnparm, char *nnret,float min_score)
        int jsonLen =0;

        yolov4_show_img_func(yuv420p , hdrimage->size, cameraCfg.camWidth, cameraCfg.camHeight, scale,src, 0, &cnn1PrmSet, metaData, conf,\
    		   jsonOut, &jsonLen);

       	if( jsonLen && http->timer()>10 )
	    {
		    printf("det OK!  seqNo %d, jsonLen :%d\n", hdr.seqNo , jsonLen);
            int jpeg_len=len_jpg-sizeof(frameSpecOut);
            unsigned char * pt_buf = new unsigned char[jpeg_len*2];

            int len = ncc_bas64((unsigned char *)yuv420p,jpeg_len,pt_buf,jpeg_len*2);
            printf("len = %d\n",len);

            http->post(cpuid,version2,(char *)pt_buf);
            delete [] pt_buf;
	    }
    }
    printf("exit test main....\n");
