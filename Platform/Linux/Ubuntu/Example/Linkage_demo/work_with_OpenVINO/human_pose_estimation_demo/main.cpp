...
#include <samples/ocv_common.hpp>
>>>>>>> 6a45673e2b5233bbcb4035ab3820b456b5dbcf52
...
int main(int argc, char* argv[]) {
...
#if 0   //delete by duke 2020.11.23
        cv::VideoCapture cap;
	...
	//cap >> curr_frame;
        ...
        cv::Mat next_frame;
//        if (!cap.grab()) 
	if (!estimator.ReadVideo(curr_frame))
	{
       ...
       // Presenter presenter(FLAGS_u, static_cast<int>(cap.get(cv::CAP_PROP_FRAME_HEIGHT)) - graphSize.height - 10, graphSize);
        ...
        while (true) {
	     ...
	     if (!estimator.ReadVideo(next_frame))	
	    {
                if (next_frame.empty()) {
		    ...
		    cv::Mat showFrame;
		    float scale=0.7;
		    cv::resize(curr_frame,showFrame,cv::Size(curr_frame.cols*scale,curr_frame.rows*scale),0,0,cv::INTER_LINEAR);	
   		    cv::imshow("Human Pose Estimation on ", showFrame);	
#endif
                    t1 = std::chrono::high_resolution_clock::now();
