...
//            if( (coordinate_is_valid(x0, y0, x1, y1) ==0 )|| (label != 0))
		...
		if(label == 1)
		    cv::rectangle(outgoing_img, box, cv::Scalar(255, 0, 0), 1, 8, 0);
		else if(label == 2)
            cv::rectangle(outgoing_img, box, cv::Scalar(0, 255, 0), 1, 8, 0);
		else
		    cv::rectangle(outgoing_img, box, cv::Scalar(0, 0, 255), 1, 8, 0);
#if 0
		cv::Point origin;
		origin.x = box.x ;
		origin.y = box.y + 32;
		...
		sprintf(result, "%d_score:%d%%",label, (int)(100*score));
