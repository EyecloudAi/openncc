...
HumanPoseEstimator::HumanPoseEstimator(const std::string& modelPath,
    ...
    ncc = new NccDev(modelPath); //add by duke 2020.11.23
}
...
void HumanPoseEstimator::startCurr() {
    //requestCurr->StartAsync();  //delete by duke 2020.11.23
}

void HumanPoseEstimator::startNext() {
   // requestNext->StartAsync();  //delete by duke 2020.11.23
}

bool HumanPoseEstimator::readyCurr() {
	return true;//edit by duke 2020.11.23
...
bool HumanPoseEstimator::ReadVideo(cv::Mat& frame)
{
	return ncc->ReadVideo(frame);
}

//edit by duke 2020.11.23
static float meta_buffer[1024*1024];
static std::vector<HumanPose> poses ;
std::vector<HumanPose> HumanPoseEstimator::postprocessCurr() {
	if (ncc->ReadMetaData(meta_buffer,sizeof(meta_buffer)))	
	{
	    	...
	    	InferenceEngine::SizeVector pafsDims =pafsBlob->getTensorDesc().getDims();
    		//printf("pasfDims   (%d, %d, %d, %d)\n",(int)pafsDims[0],(int)pafsDims[1],(int)pafsDims[2],(int)pafsDims[3]);
    		//printf("heatMapDims(%d, %d, %d, %d)\n",(int)heatMapDims[0],(int)heatMapDims[1],(int)heatMapDims[2],(int)heatMapDims[3]);

	    	char* pafs=(char*)meta_buffer;
  		char* pheat=pafs+pafsDims[0]*pafsDims[1]*pafsDims[2]*pafsDims[3]*sizeof(float);	

	    	 poses = postprocess(
		    /*heatMapsBlob->buffer()*/(const float*)pheat,
		    heatMapDims[2] * heatMapDims[3],
		    keypointsNumber,
		    /*pafsBlob->buffer()*/(const float*)pafs,
		    heatMapDims[2] * heatMapDims[3],
...
HumanPoseEstimator::~HumanPoseEstimator() {
	...
	delete ncc;//add by duke 2020.11.23
}
