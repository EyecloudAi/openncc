...
Metadata Output Parsing:
    ...
    | outputSize[0]|          /////////////model_0 output, outputSize[0]
      ...
      |---------------|
      #endif
      void  vehicle_license_plate_detection_barrier(void *data, int w, int h, float scale, char *name, int nn_fov_show, Network1Par *nnParm1,Network2Par *nnParm2, char *nnret, float min_score,int ftime,int RES,char *id,bool showstate,int flow_fps)
{
    cv::Mat yuvImg;
    ...
    if(nn_fov_show)
    {
        ...
        originNN.x =oftX;
        originNN.y = oftY + 32;
        cv::putText(outgoingImage, "NCC ROI", originNN, cv::FONT_HERSHEY_COMPLEX, 1,  cv::Scalar(0,0,0xff), 2, 8, 0);
    }
    if(showstate)
    {
        /* 帧率显示 */
        ...
        cv::putText(outgoingImage, FPS, origin_fps, cv::FONT_HERSHEY_COMPLEX, 1,  cv::Scalar(0,0,0xff), 2, 8, 0);

        string FLOW_FPS="display FPS:";
        FLOW_FPS.append(std::to_string(flow_fps).c_str());
        cv::putText(outgoingImage, FLOW_FPS, origin_flow_fps, cv::FONT_HERSHEY_COMPLEX, 1,  cv::Scalar(0,0,0xff), 2, 8, 0);
        ...
        switch(RES)
        {
        case 1:cv::putText(outgoingImage,"1080P" , origin_res, cv::FONT_HERSHEY_COMPLEX, 1,  cv::Scalar(0,0,0xff), 2, 8, 0);break;
        case 2:cv::putText(outgoingImage,"4K" , origin_res, cv::FONT_HERSHEY_COMPLEX, 1,  cv::Scalar(0,0,0xff), 2, 8, 0);break;
        case 3:cv::putText(outgoingImage,"720P" , origin_res, cv::FONT_HERSHEY_COMPLEX, 1,  cv::Scalar(0,0,0xff), 2, 8, 0);break;
        }
        ...
        origin_id.x =0;
        origin_id.y =h-10;
        cv::putText(outgoingImage,id , origin_id, cv::FONT_HERSHEY_COMPLEX, 1,  cv::Scalar(0,0,0xff), 2, 8, 0);
    }
    Mat showImage;
    /* 缩放显示 */
